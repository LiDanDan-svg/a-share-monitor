...
from data import fetch_spot, fetch_minute, market_regime, radar_candidates
...
st.title('📈 A股主升浪雷达 V2.1')
st.caption('大盘环境 + 主升浪识别 + 买点/加仓/高抛/接回 + 雷达 + 回测｜研究辅助，不构成投资建议')
...
except Exception as e: regime={'score':50,'label':'异常','breadth':0,'avg_pct':0,'top':pd.DataFrame()}
a,b,c,d=st.columns(4); a.metric('大盘环境',f"{regime['score']} · {regime['label']}"); b.metric('上涨家数占比',f"{regime['breadth']:.1f}%"); c.metric('全市场平均涨跌',f"{regime['avg_pct']:.2f}%"); d.metric('扫描时间',datetime.now().strftime('%H:%M:%S'))

if scan or 'results' not in st.session_state:
    rows=[]
    for code in codes:
        try:
            s=analyze(fetch_minute(code,period),st.session_state.holdings.get(code,0),market_score=regime['score'])
            if s: rows.append({'代码':code,'操作':s.action,'综合高抛':s.sell_score,'买点':s.buy_score,'加仓':s.add_score,'接回':s.reentry_score,'评分':s.score,'现价':s.price,'VWAP':s.vwap,'乖离%':s.dev,'RSI':s.rsi,'量比':s.vol_ratio,'建议股数':s.qty,'目标接回':s.reentry,'失效价':s.invalid,'原因':'；'.join(s.reason)})
...
with st.expander('扫描全市场候选股',expanded=True):
    if st.button('🚀 扫描雷达',use_container_width=True):
        ...
        for _,row in cand.iterrows():
            ...
            try:
                s=analyze(fetch_minute(code,period),0,market_score=regime['score'])
                if s: rr.append({'代码':code,'名称':row.get('name',''),'涨跌%':row.get('pct',0),'成交额':row.get('amount',0),'买点':s.buy_score,'加仓':s.add_score,'高抛':s.sell_score,'接回':s.reentry_score,'动作':s.action,'RSI':s.rsi,'VWAP乖离%':s.dev})
