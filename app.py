...
APP_VERSION = '2.2.2'
st.set_page_config(page_title=f'A股主升浪雷达 V{APP_VERSION}', page_icon='📈', layout='wide', initial_sidebar_state='collapsed')
...
# 版本升级时清空旧会话结果，避免V2.2.1异常文本（其中可能带URL参数）继续显示。
...
st.caption('安全与限流修复版｜AllTick / Tushare｜Token脱敏｜Trial保护｜研究辅助，不构成投资建议')
...
with st.sidebar:
    ...
    access_mode = str(sec('ALLTICK_ACCESS_MODE', 'trial')).lower()
    if access_mode not in ('trial', 'paid'):
        access_mode = 'trial'
    ...
    saved_interval = float(sec('ALLTICK_INTERVAL', 10.5 if access_mode == 'trial' else 1.05))
    if access_mode == 'trial':
        interval = max(10.5, saved_interval)
        st.caption(f'AllTick模式：🔒 trial（试用安全模式）｜有效请求间隔：{interval:.1f}秒')
    else:
        interval = st.number_input('AllTick请求间隔（秒）', min_value=0.05, max_value=60.0, value=max(0.05, saved_interval), step=0.05)
        st.caption('AllTick模式：✅ paid（已授权自选股分钟K）')
...
if not summary['alltick_configured'] and not summary['tushare_configured']:
    ...
    st.info('🔒 AllTick 试用安全模式已开启：只验证大盘演示指数，不请求这9只自选股分钟K，因此不会再连续触发 429 / 604。')

with st.expander('🩺 API连接诊断', expanded=not (summary['alltick_configured'] or summary['tushare_configured'])):
    st.write(f"分钟源：**{summary['minute_provider']}** ｜ 市场源：**{summary['market_provider']}** ｜ AllTick模式：**{summary['alltick_access_mode']}**")
...
try:
    ...
    regime = {'score': 50, 'label': '行情源不可用', 'breadth': None, 'avg_pct': 0, 'source': '不可用', 'status': market_session_status(), 'error': friendly_error(e)}
...
if not summary['watchlist_scan_enabled']:
    st.info('🔒 当前未开放自选股分钟扫描。AllTick trial 仅用于指数连通性验证；升级对应套餐后，在 Streamlit Secrets 把 `ALLTICK_ACCESS_MODE = "paid"` 即可开启。')
    st.session_state.pop('results', None)
elif scan:
    rows = []
    prog = st.progress(0)
        ...
        try:
            ...
            if s:
                rows.append({
                    ...
                    'RSI': s.rsi,
                    '量比': s.vol_ratio,
                    '建议股数': s.qty if live_mode else 0,
                    '目标接回': s.reentry,
                    '失效价': s.invalid,
                    '原因': '；'.join(s.reason),
                })
...
if summary['watchlist_scan_enabled']:
    if not res.empty:
        ...
        st.info('点击左侧“立即扫描”后开始获取自选股分钟行情。本版不再在页面首次加载时自动连扫9只股票。')

st.subheader('🔥 主升浪雷达')
with st.expander('稳定全市场扫描', expanded=True):
    st.caption('全市场候选筛选需要 Tushare 实时日线权限；候选分钟深度再走已授权的分钟API。')
...
if not summary['watchlist_scan_enabled']:
    st.info('试用安全模式下不请求自选股分钟K。升级分钟行情权限后这里会自动恢复。')
...
SERVERCHAN_KEY = ""
```
升级并确认套餐已覆盖自选A股后，只需要把：
```toml
ALLTICK_ACCESS_MODE = "paid"
```
再根据套餐限频设置 `ALLTICK_INTERVAL`。''')

with help_tab:
    st.markdown('''### V2.2.2 修复内容
- **Token彻底脱敏**：不再把 `requests` 的完整URL异常直接显示到网页。
- **429友好处理**：免费/试用模式强制至少 10.5 秒请求间隔，并对指数结果做短时缓存。
- **604权限保护**：trial 模式主动关闭自选股分钟K，不再用9只股票反复试权限。
- **只验证演示指数**：试用期通过上证指数、深证成指验证 AllTick 链路。
- **取消首次自动扫股**：只有用户主动点击“立即扫描”才会请求自选股。
- **旧异常清理**：升级到本版时自动清空V2.2.1会话结果，避免旧Token URL残留。
- **付费后开关简单**：升级套餐后将 `ALLTICK_ACCESS_MODE` 从 `trial` 改成 `paid` 即可恢复自选股分钟监控。

### 安全提醒
你之前的 Token 已经出现在截图中。V2.2.2 部署并确认不再泄露后，应在 AllTick 后台重新生成 Token，再把新 Token 写入 Streamlit Secrets。''')
